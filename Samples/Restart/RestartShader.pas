...
{ #version:1# (machine generated, don't edit!)
  ...
  Cmdline: sokol-shdc --input RestartShader.glsl --output RestartShader.pas

  Overview:

    Shader program 'restart':
      Get shader desc: RestartShaderDesc()
      Vertex shader: vs
        Attribute slots:
          ATTR_VS_POS = 0
          ATTR_VS_TEXCOORD0 = 1
  ...
  Shader descriptor records:
    ...
    var RestartShader := TShader.Create(RestartShaderDesc);
...
begin
  ...
  GRestartShaderDesc.Attrs[0].Init('pos', 'TEXCOORD', 0);
  GRestartShaderDesc.Attrs[1].Init('texcoord0', 'TEXCOORD', 1);

  case TGfx.Backend of
    {$IFDEF SOKOL_GLCORE33}
    TBackend.GLCore33:
      begin
        GRestartShaderDesc.VS.Source := VS_SOURCE_GLSL330;
        GRestartShaderDesc.FS.Source := FS_SOURCE_GLSL330;
      end;
    {$ENDIF}
    ...
    TBackend.Gles2:
      begin
        GRestartShaderDesc.VS.Source := VS_SOURCE_GLSL100;
        GRestartShaderDesc.FS.Source := FS_SOURCE_GLSL100;
      end;
    {$ENDIF}
    ...
    TBackend.Gles3:
      begin
        GRestartShaderDesc.VS.Source := VS_SOURCE_GLSL300ES;
        GRestartShaderDesc.FS.Source := FS_SOURCE_GLSL300ES;
      end;
    {$ENDIF}
    ...
    TBackend.D3D11:
      begin
        GRestartShaderDesc.VS.Source := VS_SOURCE_HLSL5;
        GRestartShaderDesc.FS.Source := FS_SOURCE_HLSL5;
      end;
    {$ENDIF}
    ...
    TBackend.MetalMacOS:
      begin
        GRestartShaderDesc.VS.Source := VS_SOURCE_METAL_MACOS;
        GRestartShaderDesc.FS.Source := FS_SOURCE_METAL_MACOS;
      end;
    {$ENDIF}
    ...
    TBackend.MetalIOS:
      begin
        GRestartShaderDesc.VS.Source := VS_SOURCE_METAL_IOS;
        GRestartShaderDesc.FS.Source := FS_SOURCE_METAL_IOS;
      end;
    {$ENDIF}
  else
    Assert(False)
  ...
  GRestartShaderDesc.vs.uniform_blocks[0].size := 64;
  GRestartShaderDesc.vs.uniform_blocks[0].layout := _SG_UNIFORMLAYOUT_STD140;
  if (TGfx.Backend.IsGL) then
    GRestartShaderDesc.vs.uniform_blocks[0].uniforms[0].Init('vs_params', _SG_UNIFORMTYPE_FLOAT4, 4);
  GRestartShaderDesc.fs.images[0].Init('tex', _SG_IMAGETYPE_2D, _SG_SAMPLERTYPE_FLOAT);
  GRestartShaderDesc.&label := 'RestartShader';
end;

function RestartShaderDesc: PNativeShaderDesc;
begin
  if (GRestartShaderDesc.VS.Entry = nil) then

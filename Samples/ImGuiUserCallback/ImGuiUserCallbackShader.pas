...
{ #version:1# (machine generated, don't edit!)
  ...
  Cmdline: sokol-shdc --input ImGuiUserCallbackShader.glsl --output ImGuiUserCallbackShader.pas

  Overview:

    Shader program 'scene':
      Get shader desc: SceneShaderDesc()
      Vertex shader: vs
        Attribute slots:
          ATTR_VS_POSITION = 0
          ATTR_VS_COLOR0 = 1
  ...
  Shader descriptor records:
    ...
    var SceneShader := TShader.Create(SceneShaderDesc);
...
begin
  ...
  GSceneShaderDesc.Attrs[0].Init('position', 'TEXCOORD', 0);
  GSceneShaderDesc.Attrs[1].Init('color0', 'TEXCOORD', 1);

  case TGfx.Backend of
    {$IFDEF SOKOL_GLCORE33}
    TBackend.GLCore33:
      begin
        GSceneShaderDesc.VS.Source := VS_SOURCE_GLSL330;
        GSceneShaderDesc.FS.Source := FS_SOURCE_GLSL330;
      end;
    {$ENDIF}
    ...
    TBackend.Gles2:
      begin
        GSceneShaderDesc.VS.Source := VS_SOURCE_GLSL100;
        GSceneShaderDesc.FS.Source := FS_SOURCE_GLSL100;
      end;
    {$ENDIF}
    ...
    TBackend.Gles3:
      begin
        GSceneShaderDesc.VS.Source := VS_SOURCE_GLSL300ES;
        GSceneShaderDesc.FS.Source := FS_SOURCE_GLSL300ES;
      end;
    {$ENDIF}
    ...
    TBackend.D3D11:
      begin
        GSceneShaderDesc.VS.Source := VS_SOURCE_HLSL5;
        GSceneShaderDesc.FS.Source := FS_SOURCE_HLSL5;
      end;
    {$ENDIF}
    ...
    TBackend.MetalMacOS:
      begin
        GSceneShaderDesc.VS.Source := VS_SOURCE_METAL_MACOS;
        GSceneShaderDesc.FS.Source := FS_SOURCE_METAL_MACOS;
      end;
    {$ENDIF}
    ...
    TBackend.MetalIOS:
      begin
        GSceneShaderDesc.VS.Source := VS_SOURCE_METAL_IOS;
        GSceneShaderDesc.FS.Source := FS_SOURCE_METAL_IOS;
      end;
    {$ENDIF}
  else
    Assert(False)
  ...
  GSceneShaderDesc.vs.uniform_blocks[0].size := 64;
  GSceneShaderDesc.vs.uniform_blocks[0].layout := _SG_UNIFORMLAYOUT_STD140;
  if (TGfx.Backend.IsGL) then
    GSceneShaderDesc.vs.uniform_blocks[0].uniforms[0].Init('vs_params', _SG_UNIFORMTYPE_FLOAT4, 4);
  GSceneShaderDesc.&label := 'SceneShader';
end;

function SceneShaderDesc: PNativeShaderDesc;
begin
  if (GSceneShaderDesc.VS.Entry = nil) then
    InitSceneShaderDesc;

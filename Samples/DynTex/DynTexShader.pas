...
{ #version:1# (machine generated, don't edit!)
  ...
  Cmdline: sokol-shdc --input DynTexShader.glsl --output DynTexShader.pas

  Overview:

    Shader program 'dyntex':
      Get shader desc: DyntexShaderDesc()
      Vertex shader: vs
        Attribute slots:
          ATTR_VS_POSITION = 0
          ATTR_VS_COLOR0 = 1
  ...
  Shader descriptor records:
    ...
    var DyntexShader := TShader.Create(DyntexShaderDesc);
...
begin
  ...
  GDyntexShaderDesc.Attrs[0].Init('position', 'TEXCOORD', 0);
  GDyntexShaderDesc.Attrs[1].Init('color0', 'TEXCOORD', 1);
  GDyntexShaderDesc.Attrs[2].Init('texcoord0', 'TEXCOORD', 2);

  case TGfx.Backend of
    {$IFDEF SOKOL_GLCORE33}
    TBackend.GLCore33:
      begin
        GDyntexShaderDesc.VS.Source := VS_SOURCE_GLSL330;
        GDyntexShaderDesc.FS.Source := FS_SOURCE_GLSL330;
      end;
    {$ENDIF}
    ...
    TBackend.Gles2:
      begin
        GDyntexShaderDesc.VS.Source := VS_SOURCE_GLSL100;
        GDyntexShaderDesc.FS.Source := FS_SOURCE_GLSL100;
      end;
    {$ENDIF}
    ...
    TBackend.Gles3:
      begin
        GDyntexShaderDesc.VS.Source := VS_SOURCE_GLSL300ES;
        GDyntexShaderDesc.FS.Source := FS_SOURCE_GLSL300ES;
      end;
    {$ENDIF}
    ...
    TBackend.D3D11:
      begin
        GDyntexShaderDesc.VS.Source := VS_SOURCE_HLSL5;
        GDyntexShaderDesc.FS.Source := FS_SOURCE_HLSL5;
      end;
    {$ENDIF}
    ...
    TBackend.MetalMacOS:
      begin
        GDyntexShaderDesc.VS.Source := VS_SOURCE_METAL_MACOS;
        GDyntexShaderDesc.FS.Source := FS_SOURCE_METAL_MACOS;
      end;
    {$ENDIF}
    ...
    TBackend.MetalIOS:
      begin
        GDyntexShaderDesc.VS.Source := VS_SOURCE_METAL_IOS;
        GDyntexShaderDesc.FS.Source := FS_SOURCE_METAL_IOS;
      end;
    {$ENDIF}
  else
    Assert(False)
  ...
  GDyntexShaderDesc.vs.uniform_blocks[0].size := 64;
  GDyntexShaderDesc.vs.uniform_blocks[0].layout := _SG_UNIFORMLAYOUT_STD140;
  if (TGfx.Backend.IsGL) then
    GDyntexShaderDesc.vs.uniform_blocks[0].uniforms[0].Init('vs_params', _SG_UNIFORMTYPE_FLOAT4, 4);
  GDyntexShaderDesc.fs.images[0].Init('tex', _SG_IMAGETYPE_2D, _SG_SAMPLERTYPE_FLOAT);
  GDyntexShaderDesc.&label := 'DyntexShader';
end;

function DyntexShaderDesc: PNativeShaderDesc;
begin
  if (GDyntexShaderDesc.VS.Entry = nil) then

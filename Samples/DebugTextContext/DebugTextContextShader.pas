...
{ #version:1# (machine generated, don't edit!)
  ...
  Cmdline: sokol-shdc --input DebugTextContextShader.glsl --output DebugTextContextShader.pas

  Overview:

    Shader program 'debugtext_context':
      Get shader desc: DebugtextContextShaderDesc()
      Vertex shader: vs
        Attribute slots:
          ATTR_VS_POS = 0
          ATTR_VS_TEXCOORD0 = 1
  ...
  Shader descriptor records:
    ...
    var DebugtextContextShader := TShader.Create(DebugtextContextShaderDesc);
...
const
  VS_SOURCE_GLSL330 =
    ...
    '    uv = vec2(texcoord0.x, 1.0 - texcoord0.y);'#10+
    '}';
...
begin
  ...
  GDebugtextContextShaderDesc.Attrs[0].Init('pos', 'TEXCOORD', 0);
  GDebugtextContextShaderDesc.Attrs[1].Init('texcoord0', 'TEXCOORD', 1);

  case TGfx.Backend of
    {$IFDEF SOKOL_GLCORE33}
    TBackend.GLCore33:
      begin
        GDebugtextContextShaderDesc.VS.Source := VS_SOURCE_GLSL330;
        GDebugtextContextShaderDesc.FS.Source := FS_SOURCE_GLSL330;
      end;
    {$ENDIF}
    ...
    TBackend.Gles2:
      begin
        GDebugtextContextShaderDesc.VS.Source := VS_SOURCE_GLSL100;
        GDebugtextContextShaderDesc.FS.Source := FS_SOURCE_GLSL100;
      end;
    {$ENDIF}
    ...
    TBackend.Gles3:
      begin
        GDebugtextContextShaderDesc.VS.Source := VS_SOURCE_GLSL300ES;
        GDebugtextContextShaderDesc.FS.Source := FS_SOURCE_GLSL300ES;
      end;
    {$ENDIF}
    ...
    TBackend.D3D11:
      begin
        GDebugtextContextShaderDesc.VS.Source := VS_SOURCE_HLSL5;
        GDebugtextContextShaderDesc.FS.Source := FS_SOURCE_HLSL5;
      end;
    {$ENDIF}
    ...
    TBackend.MetalMacOS:
      begin
        GDebugtextContextShaderDesc.VS.Source := VS_SOURCE_METAL_MACOS;
        GDebugtextContextShaderDesc.FS.Source := FS_SOURCE_METAL_MACOS;
      end;
    {$ENDIF}
    ...
    TBackend.MetalIOS:
      begin
        GDebugtextContextShaderDesc.VS.Source := VS_SOURCE_METAL_IOS;
        GDebugtextContextShaderDesc.FS.Source := FS_SOURCE_METAL_IOS;
      end;
    {$ENDIF}
  else
    Assert(False)
  ...
  GDebugtextContextShaderDesc.vs.uniform_blocks[0].size := 64;
  GDebugtextContextShaderDesc.vs.uniform_blocks[0].layout := _SG_UNIFORMLAYOUT_STD140;
  if (TGfx.Backend.IsGL) then
    GDebugtextContextShaderDesc.vs.uniform_blocks[0].uniforms[0].Init('vs_params', _SG_UNIFORMTYPE_FLOAT4, 4);
  GDebugtextContextShaderDesc.fs.images[0].Init('tex', _SG_IMAGETYPE_2D, _SG_SAMPLERTYPE_FLOAT);
  GDebugtextContextShaderDesc.&label := 'DebugtextContextShader';
end;

function DebugtextContextShaderDesc: PNativeShaderDesc;
begin
  if (GDebugtextContextShaderDesc.VS.Entry = nil) then

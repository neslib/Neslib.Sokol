...
{ #version:1# (machine generated, don't edit!)
  ...
  Cmdline: sokol-shdc --input NonInterleavedShader.glsl --output NonInterleavedShader.pas

  Overview:

    Shader program 'noninterleaved':
      Get shader desc: NoninterleavedShaderDesc()
      Vertex shader: vs
        Attribute slots:
          ATTR_VS_POSITION = 0
          ATTR_VS_COLOR0 = 1
  ...
  Shader descriptor records:
    ...
    var NoninterleavedShader := TShader.Create(NoninterleavedShaderDesc);
...
begin
  ...
  GNoninterleavedShaderDesc.Attrs[0].Init('position', 'TEXCOORD', 0);
  GNoninterleavedShaderDesc.Attrs[1].Init('color0', 'TEXCOORD', 1);

  case TGfx.Backend of
    {$IFDEF SOKOL_GLCORE33}
    TBackend.GLCore33:
      begin
        GNoninterleavedShaderDesc.VS.Source := VS_SOURCE_GLSL330;
        GNoninterleavedShaderDesc.FS.Source := FS_SOURCE_GLSL330;
      end;
    {$ENDIF}
    ...
    TBackend.Gles2:
      begin
        GNoninterleavedShaderDesc.VS.Source := VS_SOURCE_GLSL100;
        GNoninterleavedShaderDesc.FS.Source := FS_SOURCE_GLSL100;
      end;
    {$ENDIF}
    ...
    TBackend.Gles3:
      begin
        GNoninterleavedShaderDesc.VS.Source := VS_SOURCE_GLSL300ES;
        GNoninterleavedShaderDesc.FS.Source := FS_SOURCE_GLSL300ES;
      end;
    {$ENDIF}
    ...
    TBackend.D3D11:
      begin
        GNoninterleavedShaderDesc.VS.Source := VS_SOURCE_HLSL5;
        GNoninterleavedShaderDesc.FS.Source := FS_SOURCE_HLSL5;
      end;
    {$ENDIF}
    ...
    TBackend.MetalMacOS:
      begin
        GNoninterleavedShaderDesc.VS.Source := VS_SOURCE_METAL_MACOS;
        GNoninterleavedShaderDesc.FS.Source := FS_SOURCE_METAL_MACOS;
      end;
    {$ENDIF}
    ...
    TBackend.MetalIOS:
      begin
        GNoninterleavedShaderDesc.VS.Source := VS_SOURCE_METAL_IOS;
        GNoninterleavedShaderDesc.FS.Source := FS_SOURCE_METAL_IOS;
      end;
    {$ENDIF}
  else
    Assert(False)
  ...
  GNoninterleavedShaderDesc.vs.uniform_blocks[0].size := 64;
  GNoninterleavedShaderDesc.vs.uniform_blocks[0].layout := _SG_UNIFORMLAYOUT_STD140;
  if (TGfx.Backend.IsGL) then
    GNoninterleavedShaderDesc.vs.uniform_blocks[0].uniforms[0].Init('vs_params', _SG_UNIFORMTYPE_FLOAT4, 4);
  GNoninterleavedShaderDesc.&label := 'NoninterleavedShader';
end;

function NoninterleavedShaderDesc: PNativeShaderDesc;
begin
  if (GNoninterleavedShaderDesc.VS.Entry = nil) then

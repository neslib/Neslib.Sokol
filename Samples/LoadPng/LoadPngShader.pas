...
{ #version:1# (machine generated, don't edit!)
  ...
  Cmdline: sokol-shdc --input LoadPngShader.glsl --output LoadPngShader.pas

  Overview:

    Shader program 'loadpng':
      Get shader desc: LoadpngShaderDesc()
      Vertex shader: vs
        Attribute slots:
          ATTR_VS_POS = 0
          ATTR_VS_TEXCOORD0 = 1
  ...
  Shader descriptor records:
    ...
    var LoadpngShader := TShader.Create(LoadpngShaderDesc);
...
begin
  ...
  GLoadpngShaderDesc.Attrs[0].Init('pos', 'TEXCOORD', 0);
  GLoadpngShaderDesc.Attrs[1].Init('texcoord0', 'TEXCOORD', 1);

  case TGfx.Backend of
    {$IFDEF SOKOL_GLCORE33}
    TBackend.GLCore33:
      begin
        GLoadpngShaderDesc.VS.Source := VS_SOURCE_GLSL330;
        GLoadpngShaderDesc.FS.Source := FS_SOURCE_GLSL330;
      end;
    {$ENDIF}
    ...
    TBackend.Gles2:
      begin
        GLoadpngShaderDesc.VS.Source := VS_SOURCE_GLSL100;
        GLoadpngShaderDesc.FS.Source := FS_SOURCE_GLSL100;
      end;
    {$ENDIF}
    ...
    TBackend.Gles3:
      begin
        GLoadpngShaderDesc.VS.Source := VS_SOURCE_GLSL300ES;
        GLoadpngShaderDesc.FS.Source := FS_SOURCE_GLSL300ES;
      end;
    {$ENDIF}
    ...
    TBackend.D3D11:
      begin
        GLoadpngShaderDesc.VS.Source := VS_SOURCE_HLSL5;
        GLoadpngShaderDesc.FS.Source := FS_SOURCE_HLSL5;
      end;
    {$ENDIF}
    ...
    TBackend.MetalMacOS:
      begin
        GLoadpngShaderDesc.VS.Source := VS_SOURCE_METAL_MACOS;
        GLoadpngShaderDesc.FS.Source := FS_SOURCE_METAL_MACOS;
      end;
    {$ENDIF}
    ...
    TBackend.MetalIOS:
      begin
        GLoadpngShaderDesc.VS.Source := VS_SOURCE_METAL_IOS;
        GLoadpngShaderDesc.FS.Source := FS_SOURCE_METAL_IOS;
      end;
    {$ENDIF}
  else
    Assert(False)
  ...
  GLoadpngShaderDesc.vs.uniform_blocks[0].size := 64;
  GLoadpngShaderDesc.vs.uniform_blocks[0].layout := _SG_UNIFORMLAYOUT_STD140;
  if (TGfx.Backend.IsGL) then
    GLoadpngShaderDesc.vs.uniform_blocks[0].uniforms[0].Init('vs_params', _SG_UNIFORMTYPE_FLOAT4, 4);
  GLoadpngShaderDesc.fs.images[0].Init('tex', _SG_IMAGETYPE_2D, _SG_SAMPLERTYPE_FLOAT);
  GLoadpngShaderDesc.&label := 'LoadpngShader';
end;

function LoadpngShaderDesc: PNativeShaderDesc;
begin
  if (GLoadpngShaderDesc.VS.Entry = nil) then
